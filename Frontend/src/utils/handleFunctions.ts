...
export const handleSave = async (
  ...
  workflowId,
  workflowTitle,
  savedCredentials: Pick<Credential, "createdAt" | "platform" | "id">[]
) => {
  ...
  if (!res.data.ok) {
    toast.error(
      "workflow not saved due to wrong structure of nodes or connections"
    );
    return;
  }
  //updating enabled option
  if (workflowIndex !== -1) {
