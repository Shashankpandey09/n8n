...
async function getOrCreateTransporter(
  userID: number
): Promise<nodemailer.Transporter> {
  try {
    const cachedTransporter =
      CredManager.getInstance().transporterCacheGet(userID);
...
export async function sendEmail(
  ...
  try {
    ...
    const mailOptions = {
      from: `"${from}" <${
        CredManager.getInstance().getCred("smtp")?.EMAIL_USER
      }>`,
      to: to,
    ...
    if (action === "Send&wait") {
      try {
        ...
        console.log(
          `Created EmailWait entry, now waiting for a reply to: ${sentMessageId}`
        );
