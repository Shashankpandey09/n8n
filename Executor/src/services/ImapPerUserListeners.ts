...
async function processMailAndResume(source: Buffer, userId: number,messageId:string) {
    try {
        ...
        console.log(`Matched reply to waiting workflow! Resuming execution ID: ${waitEntry.executionId}`);
        ...
        const resumePayload = {
            ...
            ExecutionPayload: JSON.stringify({
                message: (typeof parsed.text === "string" && parsed.text.length > 0)
      ? parsed.text
...
async function fetchAndProcessUnseen(client: ImapFlow, userId: number) {
    try {
     const mbInfo=   await client.mailboxOpen("INBOX",{readOnly:false});
        // const uids = await client.search({ seen: false });
        // if (!uids) return;
        //@ts-ignore
 console.log("DEBUG: mailbox info:", { name: mbInfo.name, uidValidity: mbInfo.uidValidity, exists: mbInfo.exists });
  ...
  select: { id: true, messageId: true, executionId: true, workflowId: true }
});
        ...
        const uids = await client.search({ seen: false,since: new Date(Date.now() - 24*3600*1000)});
        ...
        console.log(`Found ${uids.length} unseen messages for user ${userId}.`);
         ...
         const UIDs:number[]=[]
        for await (const message of client.fetch(uids, {uid: true,headers: ['in-reply-to']})) {
...
//     const testUid = uuidsToProcess[0];
// let gotHeader = false;
// for await (const m of client.fetch([testUid], { uid: true, headers: ["in-reply-to", "subject", "from"] })) {
//   gotHeader = true;
//   console.log("DEBUG: single-header fetch keys:", Object.keys(m as any), "uid:", (m as any).uid);
//   console.log("DEBUG: in-reply-to header:", (m as any).headers?.get?.("in-reply-to") ?? m.headers?.toString?.());
// }
// if (!gotHeader) console.warn("DEBUG: single-header fetch returned NO ITEMS for uid:", testUid);

// 4) Try a raw IMAP request for the UID (this prints server response / errors)
// try {

//   const found = await client.search({ uid: testUid});
// console.log("DEBUG: search by uid 30268 =>", found);
 
// } catch (err) {
//   console.error("DEBUG: raw UID FETCH threw:", err);
// }
        ...
        for await (const msg of client.fetch(uuidsToProcess,{source:true})){
           ...
           if(msg.source){
            ...
            const message=messageIdToProcess.find((e)=>e.id===Number(msg.uid))
