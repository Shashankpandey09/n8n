...
type CreateWorkflowResult =
  ...
  | { ok: true; workflow: any; runnable: false; warnings?: any; reason?: string }
  | { ok: false; errors: any[]; message?: string }

export async function createWorkflow(userId: number, body: any): Promise<CreateWorkflowResult> {
  try {
    ...
    const structuralErrors = (normalized.err || []).filter((e: any) => structuralTypes.has(e.type));
    const otherErrors = (normalized.err || []).filter((e: any) => !structuralTypes.has(e.type));
    ...
    if (structuralErrors.length > 0) {
      return { ok: false, errors: structuralErrors, message: "Structural validation failed" };
    }

    // If there are missing required params (warnings), we can still save but keep disabled
    const hasMissingParams = Object.keys(normalized.missingParams || {}).length > 0;
 

    // Determine whether workflow is runnable: true only when no missing params and no other errors
    const runnable = !hasMissingParams && (otherErrors.length === 0)&&normalized.missingCredId.size==0;
    ...
    if (!runnable) {
      ...
      return {
        ...
        reason: hasMissingParams ? "missing_required_parameters" : "other_nonblocking_issues",
      };
    }
    ...
    return { ok: false, errors: [{ message: err?.message || String(err) }], message: "Internal error while creating workflow" };
  }
}
